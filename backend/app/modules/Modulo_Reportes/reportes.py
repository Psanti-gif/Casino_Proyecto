...
def ReporteGrupal(
    ...
    df_merged = df_filtrada.merge(
        df_maquinas_filtradas, left_on='maquina_id', right_on='ID')
    ...
    resumen['UTILIDAD'] = resumen['IN'] - resumen['OUT'] - \
        resumen['JACKPOT'] - resumen['BILLETERO']
...
def ExportToPDF(
    ...
    pdf.cell(
        200, 10, txt=f"Tipo de reporte: {tipo_reporte.upper()}", ln=True, align='C')
    pdf.cell(
        200, 10, txt=f"Rango de fechas: {fecha_inicio.date()} a {fecha_fin.date()}", ln=True, align='C')
    pdf.ln(10)
    ...
    if tipo_reporte == 'consolidado':
        ...
        for mid in maquina_ids:
            ...
            pdf.cell(
                200, 10, txt=f'Máquina ID: {mid} - {maquina["Nombre"]}', ln=True)
            pdf.set_font('Arial', 'B', size=12)
            pdf.cell(
                200, 10, txt=f'Marca: {maquina["Marca"]}, Modelo: {maquina["Modelo"]}', ln=True)
            pdf.cell(
                200, 10, txt=f'IN: {datos["IN"].sum()}, OUT: {datos["OUT"].sum()}, JACKPOT: {datos["JACKPOT"].sum()}, BILLETERO: {datos["BILLETERO"].sum()}', ln=True)
            utilidad = datos['IN'].sum() - datos['OUT'].sum() - \
                datos['JACKPOT'].sum() - datos['BILLETERO'].sum()
            pdf.cell(200, 10, txt=f'UTILIDAD: {utilidad}', ln=True)
            pdf.ln(5)

    elif tipo_reporte == 'grupal':
        df_maquinas_filtradas = df_maquinas[df_maquinas['ID'].isin(
            maquina_ids)]
        df_merged = df_filtrada.merge(
            df_maquinas_filtradas, left_on='maquina_id', right_on='ID')
        resumen = df_merged.groupby('Tipo').agg({
        ...
        for _, fila in resumen.iterrows():
            ...
            pdf.cell(
                200, 10, txt=f"IN: {fila['IN']}, OUT: {fila['OUT']}, JACKPOT: {fila['JACKPOT']}, BILLETERO: {fila['BILLETERO']}", ln=True)
...
def EnviarCorreo(data: correoData):
    ...
    with open(data.archivo_adjunto, 'rb') as f:
        nombre_archivo = data.nombre_mostrado or os.path.basename(
            data.archivo_adjunto)
        msg.add_attachment(f.read(), maintype='application',
                           subtype='octet-stream', filename=nombre_archivo)
    try:
...
def generar_reporte_participacion(data: ParticipacionRequest):
    ...
    detalle_maquina = df_maquinas[df_maquinas['ID'].isin(
        data.maquinas_seleccionadas)].copy()
